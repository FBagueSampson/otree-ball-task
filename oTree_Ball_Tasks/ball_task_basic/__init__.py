...
# ~~~ Primary settings
    ...
    NUM_BALLS = 12 # the total number of balls to allocate between the buckets
    NUM_ROUNDS = NUM_BALLS # default: number of balls determines number of rounds
    TASK_IS_TIMED = False # if use a hard-timer = True; if no timer = False
# ~~~ Secondary (time) settings
    # default: these constants required even if TASK_IS_TIMED = False
    TIME_FOR_TASK = 30 # length of time to complete ball task (hard timer)
    TIMED_IN_MINUTES = False # units for TIME_FOR_TASK: minutes = True; seconds = False
...
# ~~~ Decisions during Ball Task
    ...
    bucket_drop_01 = models.IntegerField(initial=0) # Records a 1 if the ball is placed in bucket 01, otherwise 0 (chosen)
    bucket_drop_02 = models.IntegerField(initial=0) # Records a 1 if the ball is placed in bucket 02, otherwise 0(chosen)
# ~~~ Outcomes
    sum_drops_bucket_01 = models.IntegerField(min=0) # records the sum of bucket 01 drops for each participant (computed)
    sum_drops_bucket_02 = models.IntegerField(min=0) # records the sum of bucket 02 drops for each participant (computed)
    donation = models.CurrencyField(min=0, initial=0) # records the value of the donation in every round (computed)
...
def package_payoff_object(player: Player):
    player.participant.paid_rounds_object = {'payoff_ball_task': sum(p.payoff for p in player.in_all_rounds()),
                                             ...
                                             'donation': cu(set_total_donation(player)),}
...
# ~~~ Grabs data for templates; reduces redundancies ~~~
...
def summarise_current_state(player: Player):
    return dict(
        balls_in_bucket01 = n_balls_in_bucket_01(player),
        balls_in_bucket02 = n_balls_in_bucket_02(player),
        bucket_01_payoffs = sum_earned_bucket_01(player),
        bucket_02_payoffs = sum_bucket_02_payoff(player),
        total_earnings = sum_bucket_01_payoff(player) + sum_bucket_02_payoff(player),
    )
...
def set_payoff(player: Player):
    if player.participant.treatments["task"] == False: # player.participant.treatments["task"] # player.task == False
        player.payoff = cu(player.bucket_drop_02*C.PAYOFF_DICTATOR/100) # if player is dictator task, paid for balls placed in bucket_02 only
    else:
        player.payoff = cu(player.bucket_drop_01*C.PAYOFF_BUCKET_01/100 + player.bucket_drop_02*C.PAYOFF_BUCKET_02/100) # if player is rule-following task, payoff for balls placed into either bucket
...
def set_donation(player: Player):
    if player.participant.treatments["task"] == False: # player.task == False:
        donation = cu((player.bucket_drop_01*C.PAYOFF_DICTATOR)/100)
    else:
        donation = cu(0)
    return donation
...
def n_balls_in_bucket_01(player: Player):
    return sum(p.bucket_drop_01 for p in player.in_all_rounds())

def n_balls_in_bucket_02(player: Player):
    return sum(p.bucket_drop_02 for p in player.in_all_rounds())

# ~~~ Cumulative sum of balls placed into each bucket
def compute_balls_per_urn(player: Player):
    player.sum_drops_bucket_01 = n_balls_in_bucket_01(player)
    player.sum_drops_bucket_02 = n_balls_in_bucket_02(player)

# ~~~ Record total player donations to charity
def set_total_donation(player: Player):
    if player.participant.treatments["task"] == False:
        return (n_balls_in_bucket_01(player)*C.PAYOFF_DICTATOR)/100
    else:
        return 0

# ~~~ Computes the total participant payoffs attributable to each bucket
...
# ~~~ Computes the total payoff/donation value in bucket 01 for display ~~~
def sum_earned_bucket_01(player: Player):
    if player.participant.treatments["task"] == False:
        return set_total_donation(player)
    else:
        return sum_bucket_01_payoff(player)
...
def set_bucket_selection(player: Player, timeout_happened):
    if timeout_happened == True:
        player.bucket = None
        player.bucket_drop_01 = 0
        player.bucket_drop_02 = 0
    else:
        if player.bucket == 'bucket_01':
            player.bucket_drop_01 = 1 # += 1
            player.bucket_drop_02 = 0 ## adjustments for server issues
        else:
            player.bucket_drop_01 = 0 ## adjustments for server issues
            player.bucket_drop_02 = 1 # += 1
...
class Ball_Task(Page):
    ...
    # Variables sent to template
        return dict (
            ...
            balls_remaining = max(C.NUM_BALLS - player.round_number + 1, 0),
            bucket_01_payoff = player_info["bucket_01_payoffs"],
            bucket_02_payoff = player_info["bucket_02_payoffs"],
            player_earnings = player_info["total_earnings"],
...
class Results(Page):
    ...
    def vars_for_template(player: Player):
        ...
        return dict(
            ...
            player_earnings = cu(player_info["total_earnings"]),
